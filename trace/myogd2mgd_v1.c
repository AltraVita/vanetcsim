...
#define USE_MY_MAP_MATCHING TRUE 
...
//			printf("new_Rep->onRoad is %ld\n", newRep->onRoad);
//			printf("newCandRoad->aRoad is %ld\n", newCandRoad->aRoad);
//			printf("newCandRoad->aRoad->id is %ld\n",newCandRoad->aRoad->id );
...
void amend_report(struct Region *aRegion, struct Trace *aTrace, double wdist, double wangle, double wlength, double wturns)
{
	...
	struct CandRoad *thisCandRoad, *nextCandRoad, *prevCandRoad=NULL, *aCandRoad, *itrCandRoad;
	struct Path *forepart, *postpart;
	int i , j, maxSpeed;
	struct Duallist choices;
	struct CandChoice *newChoice, *myCandChoice=NULL;
	double prevRep2thisRep, prevAmd2thisAmd ;
	struct Item *aItem, *itrItem;
	...
	if(USE_MY_MAP_MATCHING){
	/*find candidates and insert them into the Duallist choices.*/
		if(!is_duallist_empty(&(thisRep->candRoads))){
			itrItem = thisRep->candRoads.head;
//			while(itrItem != NULL)
				itrCandRoad = (struct CandRoad*)itrItem->datap;
				myCandChoice = (struct CandChoice*)malloc(sizeof(struct CandChoice));
				myCandChoice->score = itrCandRoad->weight; 
				myCandChoice->onRoad = itrCandRoad;
				printf("itrCandRoad->aRoad is %ld\n",itrCandRoad->aRoad); 
				duallist_add_in_sequence_from_head(&choices, myCandChoice, (int(*)(void*,void*))candchoice_has_smaller_score_than);
...
//			}		
	...
	if(myCandChoice != NULL) 
		thisRep->onRoad = myCandChoice;
	else { 
		thisRep->onRoad = NULL;
	}
	printf("thisRep->onRoad is %ld\n",thisRep->onRoad); 
	printf("thisRep->onRoad->aRoad->datap->aRoad is %ld\n",((struct CandRoad*)thisRep->onRoad->datap)->aRoad);
	printf("thisRep->onRoad->aRoad->datap->aRoad->id is %ld\n",((struct CandRoad*)thisRep->onRoad->datap)->aRoad->id);
