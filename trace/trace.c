...
void add_candidate_roads_from_modified_point(struct Cell *aCell, struct Report *aRep, int angleValid, struct Point* modifiedPoint)
{
	...
	while(aItem!=NULL) {
		...
		aCandRoad = (struct CandRoad*)malloc(sizeof(struct CandRoad));
		aCandRoad->aRoad = aRoad;
		aCandRoad->distance = distance_point_to_polyline(modifiedPoint, &aRoad->points, &(aCandRoad->gPoint), &(aCandRoad->onSegment));
...
struct Trace* load_trace_with_hashtable(int magicNumber, FILE *ftrace, struct Region *aRegion, struct Hashtable *traces, time_t *startAt, time_t *endAt, struct Box *box)
{
	...
	while(fgets(buf, 256, ftrace)) {
		...
		if(aRegion == NULL && *startAt == 0 && *endAt == 0 && box->xmin == 0) {
			needed = 1;
		} else if (aRegion == NULL && *startAt == 0 && *endAt == 0 && box->xmin) {
			if (is_point_in_box(&aPoint, box)) needed = 1; else needed = 0;
		} else if (aRegion == NULL && *startAt == 0 && *endAt && box->xmin == 0) {
			if (timestamp <= *endAt) needed = 1; else break;
		} else if (aRegion == NULL && *startAt == 0 && *endAt && box->xmin) {
			if (is_point_in_box(&aPoint, box) && timestamp <= *endAt) needed = 1;
			else if (timestamp > *endAt) break;
			else needed = 0;
		} else if (aRegion == NULL && *startAt && *endAt == 0 && box->xmin == 0) {
			if (timestamp >= *startAt) needed = 1; else needed = 0;
		} else if (aRegion == NULL && *startAt && *endAt == 0 && box->xmin) {
			if (is_point_in_box(&aPoint, box) && timestamp >= *startAt) needed = 1; else needed = 0;
		} else if (aRegion == NULL && *startAt && *endAt && box->xmin == 0) {
			if (timestamp <= *endAt && timestamp >= *startAt) needed = 1; 
			else if (timestamp > *endAt) break;
			else needed = 0;
		} else if (aRegion == NULL && *startAt && *endAt && box->xmin) {
			if (is_point_in_box(&aPoint, box) && timestamp >= *startAt && timestamp <= *endAt ) needed = 1; 
			else if (timestamp > *endAt) break;
			else needed = 0;
		} else if (aRegion && *startAt == 0 && *endAt == 0 && box->xmin == 0) {
			if (is_point_in_polygon(&aPoint, aRegion->chosen_polygon))
				needed = 1; else needed = 0;
		} else if (aRegion && *startAt == 0 && *endAt == 0 && box->xmin) {
			if (is_point_in_box(&aPoint, box) && is_point_in_polygon(&aPoint, aRegion->chosen_polygon))
				needed = 1; else needed = 0;
		} else if (aRegion && *startAt == 0 && *endAt && box->xmin == 0) {
			if (timestamp <= *endAt && is_point_in_polygon(&aPoint, aRegion->chosen_polygon)) needed = 1; 
			else if (timestamp > *endAt) break;
			else needed = 0;
		} else if (aRegion && *startAt == 0 && *endAt && box->xmin) {
			if (timestamp <= *endAt && is_point_in_box(&aPoint, box) && is_point_in_polygon(&aPoint, aRegion->chosen_polygon)) needed = 1; 
			else if (timestamp > *endAt) break;
			else needed = 0;
		} else if (aRegion && *startAt && *endAt == 0 && box->xmin == 0) {
			if (is_point_in_polygon(&aPoint, aRegion->chosen_polygon) && timestamp >= *startAt)
				needed = 1; else needed = 0;
		} else if (aRegion && *startAt && *endAt == 0 && box->xmin) {
			if (is_point_in_polygon(&aPoint, aRegion->chosen_polygon) && timestamp >= *startAt && is_point_in_box(&aPoint, box))
				needed = 1; else needed = 0;
		} else if (aRegion && *startAt && *endAt && box->xmin == 0) {
			if (is_point_in_polygon(&aPoint, aRegion->chosen_polygon) && timestamp >= *startAt && timestamp <= *endAt) needed = 1; 
			else if (timestamp > *endAt) break;
			else needed = 0;
		} else if (aRegion && *startAt && *endAt && box->xmin) {
			if (is_point_in_polygon(&aPoint, aRegion->chosen_polygon) && timestamp >= *startAt && timestamp <= *endAt && is_point_in_box(&aPoint, box)) needed = 1; 
			else if (timestamp > *endAt) break;
			else needed = 0;
		} 
		if( needed ) {	
			...
			aItem = hashtable_find(traces, vName);
			if(aItem == NULL) {
				//printf("%s\n", vName);
...
void trace_init_func(struct Trace *aTrace)
{
	...
	duallist_init(&aTrace->transVecHistory);
}
...
void trace_free_func(struct Trace *aTrace)
{
	...
	duallist_destroy(&aTrace->transVecHistory,free);
	free(aTrace);
}
...
//added by fwu, set the transVec of the trace.
void set_trace_transVec(struct Trace* aTrace, double x, double y){
	struct Point* pTransVec = (struct Point*)malloc(sizeof(struct Point));
	pTransVec->x = x;
	pTransVec->y = y;
	duallist_add_to_head(&aTrace->transVecHistory, (void*)pTransVec);
	struct Point* testVec = (struct Point*)aTrace->transVecHistory.head->datap;
	assert(!is_duallist_empty(&aTrace->transVecHistory));
}
//added by fwu, get the transVec of the trace.
struct Point* get_trace_transVec(struct Trace* aTrace){
	return (struct Point*)aTrace->transVecHistory.head->datap;
}

//added by fwu, dump the transVecHistory
void trace_dump_transVecHistory(FILE* dumpfile, struct Trace* aTrace){
	struct Point* testtransvec = (struct Point*)aTrace->transVecHistory.head->datap;
	duallist_dump(dumpfile, &aTrace->transVecHistory, dump_transVec);
}

void dump_transVec(FILE* dumpfile, void* item){
	struct Point* ptransVec = (struct Point*)((struct Item*) item)->datap;
	fprintf(dumpfile, "(%lf, %lf)\n", ptransVec->x, ptransVec->y);
}
