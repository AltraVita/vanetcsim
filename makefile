...
INCLUDES= \
	...
	-I/usr/include/freetype2 -I/usr/include/libpng12 \
 	-I./trace -I./trace/bus -I./common -I./geometry -I./GUI -I./GUI/color -I./mobility \
...
LIBS	= \
	-L/lib -lgtk-x11-2.0 -lgdk-x11-2.0 -latk-1.0 -lgdk_pixbuf-2.0 \
...
OBJ_MYOGD2MGD = \
	common/common.o common/files.o \
	geometry/geometry.o \
	trace/trace.o trace/bus/busroute.o trace/myogd2mgd.o 
OBJ_INSERTMGD = \
	common/common.o common/files.o \
	geometry/geometry.o \
...
CLEANFILES= \
	mapviewer maptuner pickshgps splitshgps dist contactfinder dividelist comset pair_filter vmobility ict_redundency ict cutgd \
	genmgd getpdf redefact strtot getdly getfpos getRoutesCoverage getRegionArea parsePkgDump getTraceBox getavg picktxt sztaxi2ogd sztaxi2txt insertmgd ogd2mgd myogd2mgd\
	run_busnet run_adhoc_fwding getroute rmdefact \
	...
	geometry/geometry.o geometry/maptuner.o trace/bus/busroute.o trace/bus/getroute.o \
	trace/trace.o trace/convertor/shanghai/pickgps.o trace/genmgd.o trace/convertor/shanghai/splitshgps.o trace/insertmgd.o \
	trace/cutgd.o trace/convertor/shenzhen/sztaxi2ogd.o trace/convertor/shenzhen/sztaxi2txt.o trace/ogd2mgd.o trace/myogd2mgd.o trace/convertor/shanghai/pickshgps.o \
	trace/convertor/shanghai/shgps.o \
	GUI/mapviewer.o GUI/gtk_cairo.o GUI/events.o \
	mobility/contact.o mobility/mobility.o mobility/contactfinder.o mobility/vmobility.o mobility/ict_redundency.o \
	mobility/ict.o mobility/pair_filter.o mobility/ict_nmi.o \
	tools/comset.o tools/dividelist.o tools/getpdf.o tools/rmdefact.o tools/dist.o tools/getdly.o tools/strtot.o \
	tools/getfpos.o tools/parsePkgDump.o tools/getTraceBox.o tools/getRoutesCoverage tools/getRegionBox tools/getavg.o tools/getRegionArea.o tools/getRoutesCoverage.o\
	...
	simulation/oracle/oracle.o simulation/traffic/traffic.o simulation/exprs/run_busnet.o simulation/exprs/run_adhoc_fwding.o \
...
all:	clean mapviewer maptuner pickshgps splitshgps dist contactfinder getroute \
	...
	sztaxi2ogd sztaxi2txt insertmgd ogd2mgd myogd2mgd rsutime checktraj spooftraj sybilrst logtraj vcover rsutopo dumpruns checkfee dumprep run_busnet run_adhoc_fwding
...
myogd2mgd: $(OBJ_MYOGD2MGD)
	$(LINK) $(LDFLAGS) $(LDOUT)$@ $(OBJ_MYOGD2MGD)  $(LIBS)
...
install: 
	$(INSTALL) -m 755 mapviewer maptuner pickshgps splitshgps dist contactfinder getroute \
		...
		sztaxi2ogd sztaxi2txt ogd2mgd myogd2mgd insertmgd rsutime checktraj spooftraj sybilrst logtraj vcover rsutopo dumpruns checkfee dumprep \
		run_busnet run_adhoc_fwding $(BINDEST)
	$(RM) mapviewer maptuner pickshgps splitshgps dist contactfinder getroute \
		vmobility ict_redundency ict pair_filter dividelist comset genmgd cutgd rmdefact getpdf strtot getdly getfpos getRoutesCoverage \
		getRegionArea parsePkgDump getTraceBox getavg picktxt \
		sztaxi2ogd sztaxi2txt ogd2mgd myogd2mgd insertmgd rsutime checktraj spooftraj sybilrst logtraj vcover rsutopo dumpruns checkfee dumprep \
